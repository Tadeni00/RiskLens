...
class FraudTrapUser(HttpUser):
    ...
    def _random_transaction(self):
        ...
        return {
            ...
            "amount": round(random.lognormvariate(9, 1.5), 2),  # Realistic amount distribution
            "currency": "NGN",
            ...
            "merchant_id": random.choice(self.merchant_pool) if random.random() > 0.2 else None,
            "country_code": "NG" if random.random() > 0.1 else "US",
            "ip_address_hash": f"ip_{random.randint(1, 10000)}" if random.random() > 0.3 else None,
            "latitude": round(random.uniform(6.0, 7.0), 4) if random.random() > 0.5 else None,
            "longitude": round(random.uniform(3.0, 4.0), 4) if random.random() > 0.5 else None,
        }
            ...
            if response.status_code == 200:
                try:
                    ...
                    if data.get("decision") in ("REVIEW", "BLOCK"):
                        ...
                        response.failure(f"Expected REVIEW/BLOCK, got {data.get('decision')}")
                except Exception:
                    response.failure(f"Invalid JSON: {response.text}")
            else:
    ...
    def batch_score(self):
        ...
        with self.client.post("/v1/score/batch", json=batch, catch_response=True) as response:
            if response.status_code == 200:
                try:
                    data = response.json()
                    if isinstance(data, list) and len(data) == 50:
                        ...
                        response.failure(f"Invalid batch response: {len(data) if isinstance(data, list) else 'not list'}")
                except Exception:
                    response.failure(f"Invalid JSON: {response.text}")
            else:
                response.failure(f"HTTP {response.status_code}")
