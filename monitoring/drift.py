...
def compute_kl_divergence(
    baseline: np.ndarray, current: np.ndarray, bins: int = 20
) -> float:
    """
...
def compute_embedding_drift(
    ...
    if len(current_embeddings) > 0:
        # Normalize
        curr_norm = current_embeddings / np.linalg.norm(
            current_embeddings, axis=1, keepdims=True
        )
...
def detect_feature_outliers(
    values: np.ndarray, method: str = "iqr", threshold: float = 3.0
) -> np.ndarray:
    """
