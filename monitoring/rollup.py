...
def fetch_recent_scores(
    ...
    cols = ["tenant_id", "transaction_id", "risk_score", "decision", 
            "model_phase", "model_version", "latency_ms", "scored_at", "is_fraud"] + \
           DEFAULT_MONITORED_FEATURES
...
def fetch_labels(
    ...
    if not rows:
        return pd.DataFrame()
    return pd.DataFrame(rows, columns=["transaction_id", "is_fraud", "risk_score", "scored_at"])
...
def insert_drift_metrics(ch: Client, drift_results: dict, tenant_id: str, bucket_date: date, bucket_hour: int = 0) -> None:
    ...
    ch.execute(
        "INSERT INTO drift_metrics_hourly VALUES",
        data
    )


def insert_concept_drift(ch: Client, concept_result, tenant_id: str, bucket_date: date) -> None:
...
def insert_embedding_drift(ch: Client, embed_result, tenant_id: str, bucket_date: date, bucket_hour: int = 0) -> None:
...
def process_tenant(ch: Client, tenant_id: str, bucket_date: date, bucket_hour: int = 0) -> None:
    """Process a single tenant's daily metrics."""
    logger.info("Processing daily metrics for tenant={} date={}", tenant_id, bucket_date)
    ...
    try:
        ...
        model_version = df["model_version"].mode().iloc[0] if not df["model_version"].isna().all() else "unknown"
        model_phase = df["model_phase"].mode().iloc[0] if not df["model_phase"].isna().all() else "UNKNOWN"
