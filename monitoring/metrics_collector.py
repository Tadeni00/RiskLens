...
def fetch_recent_scores(
    ...
    try:
        ...
        return pd.DataFrame(rows, columns=[
            "tenant_id", "transaction_id", "risk_score", "decision",
            "model_phase", "model_version", "latency_ms", "is_fraud", "scored_at"
        ])
...
def compute_daily_metrics(
    ...
    model_version = scores["model_version"].mode()[0] if not scores["model_version"].empty else model_version
    model_phase = scores["model_phase"].mode()[0] if not scores["model_phase"].empty else model_phase
    
    # Join with labels if available
    if not labels_df.empty:
        labels = labels_df[labels_df["transaction_id"].isin(scores["transaction_id"])]
        scores = scores.merge(labels, on="transaction_id", how="left", suffixes=("", "_label"))
    else:
        scores["label"] = np.nan
    ...
    if labeled_mask.sum() > 10:
        ...
        from sklearn.metrics import average_precision_score, roc_auc_score, recall_score, precision_score, fbeta_score
...
def persist_daily_metrics(client: Client, metrics: DailyModelMetrics) -> None:
    ...
    """, [[
        metrics.tenant_id, metrics.model_version, metrics.model_phase, metrics.bucket_date,
        metrics.auc_pr, metrics.auc_roc, metrics.recall, metrics.precision,
        metrics.f2_score, metrics.fraud_capture_rate, metrics.review_rate,
        metrics.fpr, metrics.avg_latency_ms, metrics.p95_latency_ms,
        metrics.total_scored, metrics.total_blocked, metrics.total_reviewed,
        metrics.total_approved, metrics.max_psi, metrics.max_kl,
...
def persist_drift_metrics(
    ...
    for feature, result in drift_results.items():
        rows.append([
            tenant_id, feature, "psi", result.psi, bucket_date, bucket_hour,
            1 if result.drift_detected else 0
        ])
        rows.append([
            tenant_id, feature, "kl", result.kl_divergence, bucket_date, bucket_hour,
            1 if result.drift_detected else 0
        ])
...
def persist_embedding_drift(
    ...
    """, [[
        result.tenant_id, bucket_date, bucket_hour,
        result.centroid_distance, result.max_distance, result.mean_distance,
        1 if result.drift_detected else 0
    ]])
...
def persist_concept_drift(
    ...
    """, [[
        result.tenant_id, bucket_date,
        result.label_rate_baseline, result.label_rate_current,
        result.rate_change, 1 if result.drift_detected else 0,
        result.prediction_rate_baseline, result.prediction_rate_current
    ]])
...
def run_daily_rollup(tenant_ids: list[str], bucket_date: Optional[date] = None) -> None:
    ...
    for tenant_id in tenant_ids:
        logger.info("Running daily rollup for tenant={} date={}", tenant_id, bucket_date)
        ...
        try:
            ...
            feature_list = [f for f in DEFAULT_MONITORED_FEATURES if f in scores_df.columns]
            ...
            if len(baseline_embeddings) > 0 and len(current_embeddings) > 0:
                ...
                persist_embedding_drift(client, embedding_drift, bucket_date, bucket_hour)
            
            # 4. Compute concept drift
            concept_drift = None
            if not labels_df.empty:
                # Split labels by period
                labels_df["labelled_date"] = pd.to_datetime(labels_df["labelled_at"]).dt.date
                base_labels = labels_df[
                    (labels_df["labelled_date"] >= baseline_start) & 
                    (labels_df["labelled_date"] <= baseline_end)
                ]["label"].values
                curr_labels = labels_df[
                    labels_df["labelled_date"] == current_day
                ]["label"].values
                
                # Get predictions for same transactions
                base_scores = scores_df[scores_df["scored_date"].isin(
                    pd.date_range(baseline_start, baseline_end)
                )]["risk_score"].values
                curr_scores = scores_df[scores_df["scored_date"] == current_day]["risk_score"].values
                
                concept_drift = compute_concept_drift(
                    tenant_id, base_labels, curr_labels,
            ...
            metrics = compute_daily_metrics(
                ...
                model_version=scores_df["model_version"].mode()[0] if not scores_df["model_version"].empty else "unknown",
                model_phase=scores_df["model_phase"].mode()[0] if not scores_df["model_phase"].empty else "unknown",
                bucket_date=bucket_date
            )
            ...
            persist_drift_metrics(client, tenant_id, drift_results, bucket_date, bucket_hour)
            
            logger.info("Daily rollup complete for tenant={}: scored={}, drift_features={}",
                       tenant_id, metrics.total_scored if metrics else 0, metrics.features_with_drift if metrics else 0)
