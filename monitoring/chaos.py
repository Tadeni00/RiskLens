...
class ChaosRunner:
    ...
    def register_injection_point(self, name: str, inject_fn: Callable, rollback_fn: Callable) -> None:
    ...
    def run_experiment(self, experiment_key: str) -> dict:
        ...
        try:
            ...
            injection["inject"]()
            observations.append(f"Chaos injected at {datetime.now(timezone.utc).isoformat()}")
            
            # Wait for duration or stop signal
            if stop_event.wait(timeout=CHAOS_EXPERIMENTS[experiment_key]["duration_seconds"]):
                observations.append("Experiment stopped early")
            else:
                observations.append(f"Duration {CHAOS_EXPERIMENTS[experiment_key]['duration_seconds']}s completed")
            ...
            try:
                ...
                injection["rollback"]()
                observations.append(f"Rollback completed at {datetime.now(timezone.utc).isoformat()}")
...
def run_chaos_experiment(experiment_name: str) -> dict:
    ...
    runner.register_injection_point("model_artifacts", *create_model_missing_injection())
    runner.register_injection_point("feature_store", *create_high_latency_injection("feature_store", 200))
