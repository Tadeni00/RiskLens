...
class AdaptiveConfig:
    """Configuration for Adaptive Learning Layer training."""
...
class AdaptiveTrainingResult:
    """Result of an Adaptive Learning Layer training run."""
...
class AdaptiveTrainer:
    """
    Training pipeline for the Adaptive Learning Layer.
    ...
    def prepare_dataset(
        ...
        """
        if X_unlabelled is not None and len(X_unlabelled) > 0 and cold_start is not None:
            ...
            if len(X_pseudo) > 0:
                ...
                w_confirmed = np.full(len(y_confirmed), self.config.confirmed_label_weight)
        ...
        logger.info(
            "Dataset prepared: %d confirmed + %d pseudo = %d total " "(fraud rate: %.3f%%)",
            len(y_confirmed),
    ...
    def train(
    ...
    ) -> Tuple[TabPFNAdaptiveLearner, AdaptiveTrainingResult]:
        """
        Fit TabPFN on the labelled dataset.

        Returns:
            (fitted learner, training result)
        """
        t_start = time.time()
        ...
        try:
            ...
            learner.fit(X_train, y_train, sample_weights=None)

            val_probs_raw = learner.predict_proba(X_val)

            learner.calibrator = ProbabilityCalibrator(method=self.config.calibration_method)
            learner.calibrator.fit(val_probs_raw, y_val)

            val_probs_cal = learner.calibrator.transform(val_probs_raw)
            result.pr_auc = float(average_precision_score(y_val, val_probs_cal))
            result.roc_auc = float(roc_auc_score(y_val, val_probs_cal))

            fraction_pos, mean_predicted = calibration_curve(y_val, val_probs_cal, n_bins=10)
            result.calibration_error = float(np.mean(np.abs(fraction_pos - mean_predicted)))

            result.status = "completed"

            learner.is_fitted = True
            learner.pr_auc_ = result.pr_auc
            learner.roc_auc_ = result.roc_auc
            learner.trained_at = datetime.now(timezone.utc).isoformat()
            learner.model_version = f"v2_tabpfn_{int(time.time())}"

            result.model_version = learner.model_version

        except Exception as exc:
            result.status = "failed"
            result.error = str(exc)
            logger.error("Adaptive learner training failed: %s", exc)
            raise
        ...
        logger.info(
            "Adaptive learner trained in %.1fs — PR-AUC: %.4f, ROC-AUC: %.4f, "
