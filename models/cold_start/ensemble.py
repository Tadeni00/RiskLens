...
class ColdStartEnsemble:
    ...
    def fit(
        ...
        self.dataset_hash = hashlib.sha256(f"{X_scaled.shape}{X_scaled.mean():.6f}{X_scaled.std():.6f}".encode()).hexdigest()[:16]
        self.trained_at = datetime.now(timezone.utc).isoformat()

        self._fit_vae(X_scaled, epochs, batch_size, lr, device, early_stopping_patience, use_mixed_precision)
        ...
        logger.info("VAE threshold calibrated: {:.6f}; EVT={}", self._vae_threshold, self._vae_evt)
    ...
    def _fit_vae(
        self, X_scaled: np.ndarray, epochs: int, batch_size: int, lr: float, device: str,
        early_stopping_patience: int = 5, use_mixed_precision: bool = True
    ) -> None:
        ...
        for epoch in range(epochs):
            ...
            for i in range(0, n, batch_size):
                ...
                if scaler is not None:
                    with torch.cuda.amp.autocast():
                        ...
                        loss, _, _ = FraudVAE.loss(batch, x_hat, mu, log_var, beta=self.vae_beta)
            ...
            if avg_loss < best_loss - 1e-6:
                ...
                if patience_counter >= early_stopping_patience:
                    logger.info("Early stopping at epoch {} (best loss: {:.6f})", epoch + 1, best_loss)
                    break
    ...
    def explain(self, X: np.ndarray, top_n: int = 8) -> list[dict]:
        ...
        for i in range(n_samples):
            ...
            top_features = [
                {"feature": name, "value": float(val), "contribution": float(val), "method": "cold_start_component"}
    ...
    def _normalise_score(self, name: str, raw_scores: np.ndarray) -> np.ndarray:
        ...
        xp = np.array([cal["p50"], cal["p95"], cal["p99"], cal["p999"]], dtype=np.float32)
    ...
    def _legacy_normalise(raw_scores: np.ndarray) -> np.ndarray:
        return (raw_scores - raw_scores.min()) / (raw_scores.max() - raw_scores.min() + 1e-8)
    ...
    def explain(self, X: np.ndarray, top_n: int = 8) -> list[dict]:
        ...
        vae_raw = self._vae_scores(X_scaled, "cpu")
        iforest_raw = -self.iforest.decision_function(X_scaled)
        tail_raw = self.tail_detector.anomaly_score(X_scaled) if self.tail_detector is not None else np.zeros(n_samples)
    ...
    def load(cls, path: Path, device: str = "cpu") -> "ColdStartEnsemble":
        ...
        obj.vae.eval()
        logger.info("ColdStartEnsemble loaded from {} (version={}, training_hash={})", 
                    path, obj.model_version, obj.training_hash)
        return obj
