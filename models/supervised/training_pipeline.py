...
from models.supervised.challengers import (
    BaseChallenger, create_challenger, get_available_algorithms
)
...
class TrainingConfig:
    ...
    champion_params: Dict[str, Any] = field(default_factory=lambda: {
        "iterations": 1000,
        "depth": 6,
        "learning_rate": 0.05,
    ...
    # Challenger training
    challenger_algorithms: List[str] = field(default_factory=lambda: [
        "xgboost", "lightgbm", "ft_transformer", "tabnet"
    ])
...
class TrainingPipeline:
    ...
    def run(
        ...
        logger.info(
            ...
            run.n_samples, run.n_features, run.fraud_rate
        )
        ...
        try:
            ...
            evaluation = self._evaluate_models(
                champion_id, champion_metrics,
                challenger_ids, challenger_metrics,
            )
            ...
            logger.info(
                ...
                run.duration_seconds, run.champion_id, run.recommended_champion
            )
    ...
    def _train_champion(
        ...
        champion.fit(
            ...
            calibration_method=self.config.champion_params.get("calibration_method", "isotonic"),
        )
        ...
        logger.info(
            ...
            training_time, metrics["pr_auc"], metrics["f2_score"]
        )
    ...
    def _train_challengers(
        ...
        for algorithm in self.config.challenger_algorithms:
            ...
            try:
                ...
                # Evaluate on test set
                metrics = challenger.compute_metrics(X_test, y_test, fpr_threshold=self.config.fpr_threshold)
                ...
                logger.info(
                    ...
                    algorithm, training_time, metrics["pr_auc"], metrics["f2_score"]
                )
    ...
    def _evaluate_models(
        ...
        if best_challenger_id:
            should_promote, criteria_met, criteria_failed = self.promoter.should_promote(
                best_challenger_id,
                champion_metrics,
            ...
            return {
                ...
                "recommended_champion": best_challenger_id if should_promote else champion_id,
    ...
    def list_runs(self, limit: int = 50) -> List[TrainingRun]:
        ...
        for filepath in sorted(self.config.output_dir.glob("run_*.json"), reverse=True)[:limit]:
            with open(filepath, "r") as f:
                data = json.load(f)
