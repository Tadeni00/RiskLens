...
class NearestNeighborIndex:
    ...
    def _init_faiss(self) -> None:
        ...
        try:
            ...
            logger.info("FAISS index initialized for tenant={} (dim={})", self.tenant_id, self.n_features)
        except Exception as exc:
            logger.warning("FAISS init failed for tenant={}: {}", self.tenant_id, exc)
            self.use_faiss = False
    
    def fit(self, X: np.ndarray, transaction_ids: List[str], labels: np.ndarray) -> None:
        """
        ...
        with self._lock:
            ...
            ids_legit = [tid for tid, is_legit in zip(transaction_ids, legit_mask) if is_legit]
            
            if len(X_legit) == 0:
                logger.warning("No legitimate transactions for tenant={}", self.tenant_id)
                return
            ...
            logger.info(
                ...
                self.tenant_id, len(ids_legit),
            )
    
    def add(self, X: np.ndarray, transaction_ids: List[str], labels: np.ndarray) -> None:
    ...
    def query(self, X: np.ndarray, k: int = 1) -> List[List[NearestNeighbor]]:
        ...
        with self._lock:
            ...
            if self.use_faiss and self._index is not None:
                ...
                for i in range(len(X)):
                    distances = [
                        (j, self.distance_metric.compute(
    ...
    def _get_raw_features(self, idx: int) -> Dict[str, float]:
        ...
        return {
            self.feature_names[i]: float(self._data[idx][i] / np.sqrt(
    ...
    def _rebuild_faiss(self) -> None:
        ...
        self._index = faiss.IndexFlatL2(self.n_features)
        legit_data = np.array([
            self._data[i] for i in range(len(self._data))
            if self._labels[i] == 0
        ], dtype=np.float32)
...
class NearestNeighborCounterfactual:
    ...
    def explain(
        ...
        neighbors = index.query(X_query, k=min(self.max_neighbors, index.legitimate_count))
        ...
        changes.sort(key=lambda c: abs(c.current_value - c.counterfactual_value), reverse=True)
            ...
            if tenant_dir.is_dir():
                ...
                if index_path.exists():
                    try:
                        ...
                        logger.warning("Failed to load NN index for {}: {}", tenant_dir.name, exc)
