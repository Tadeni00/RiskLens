...
class ExplainabilityEngine:
    ...
    def __init__(self, config: Optional[ExplainabilityConfig] = None):
        ...
        self._dice_counterfactual = (
            DiCECounterfactual() if self.config.analyst_dice else None
        )
        self._formatter = ExplanationFormatter(
            max_drivers=self.config.shap_top_features
        )
    ...
    def fit(
        ...
        if (
            X_legitimate is not None
            and legitimate_ids is not None
            and len(X_legitimate) > 0
        ):
            metric = (
                WeightedDistanceMetric(self.config.feature_weights)
                if self.config.weighted_distance
                else WeightedDistanceMetric()
            )
    ...
    def explain(
        ...
        self._monitor.record_explanation(
            ...
            fraud_drivers=[
                f"{a.feature} is {a.value}"
                for a in (shap_result.top_features if shap_result else [])
            ],
            counterfactual_features=[
                c.feature for c in (cf_result.changes if cf_result else [])
            ],
        )
    ...
    def format_report(
        ...
        report = self._formatter.format(
            fraud_probability, shap, counterfactual, confidence
        )
    ...
    @classmethod
    def load(
        cls, path: Path, config: Optional[ExplainabilityConfig] = None
