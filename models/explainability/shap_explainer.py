...
class SHAPExplainer:
    ...
    def fit(self, model, X_background: np.ndarray, feature_names: List[str]) -> None:
        ...
        try:
            ...
            logger.info(
                ...
                len(feature_names), len(X_background),
            )
        except Exception as exc:
            logger.warning("SHAP TreeExplainer init failed, trying KernelExplainer: {}", exc)
            try:
    ...
    def explain(self, X: np.ndarray, top_n: Optional[int] = None) -> SHAPExplanation:
        ...
        try:
            ...
            fraud_prob = float(np.clip(
                self._get_prediction(X_input), 0.0, 1.0
            ))
    ...
    def explain_batch(self, X: np.ndarray, top_n: Optional[int] = None) -> List[SHAPExplanation]:
        """Compute SHAP explanations for a batch of transactions."""
        return [self.explain(X[i:i+1], top_n) for i in range(len(X))]
    
    def global_importance(self, X: np.ndarray, top_n: Optional[int] = None) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            return [
                {
                    "feature": self._feature_names[i] if i < len(self._feature_names) else f"feature_{i}",
