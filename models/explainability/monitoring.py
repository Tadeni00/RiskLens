...
class ExplainabilityMonitor:
    ...
    def record_explanation(
        ...
        # Keep window
        for attr in ("_shap_latencies", "_cf_latencies", "_total_latencies", "_cache_hits", "_cf_successes"):
    ...
    def get_metrics(self) -> ExplainabilityMetrics:
        ...
        shap_lats = np.array(self._shap_latencies) if self._shap_latencies else np.array([0.0])
        cf_lats = np.array(self._cf_latencies) if self._cf_latencies else np.array([0.0])
        
        cache_hit_rate = (
            np.mean(self._cache_hits) if self._cache_hits else 0.0
        )
        cf_success_rate = (
            np.mean(self._cf_successes) if self._cf_successes else 0.0
        )
        ...
        return ExplainabilityMetrics(
            ...
            most_common_counterfactual_features=[{"feature": f, "count": c} for f, c in sorted_cf],
