...
class DiCECounterfactual:
    ...
    def fit(
        ...
        try:
            ...
            df_bg = pd.DataFrame(X_background[:min(500, len(X_background))], columns=feature_names)
            data = dice_ml.Data(
                dataframe=df_bg,
            ...
            logger.info("DiCE counterfactual engine fitted (features={})", len(feature_names))
    ...
    def generate(
        ...
        try:
            ...
            # Compute distance
            cf_features = np.array([cf_row.get(f, 0.0) for f in self._feature_names], dtype=np.float32)
    ...
    def generate_with_model(
        ...
        changes = []
        for i, fname in enumerate(self._feature_names if hasattr(self, '_feature_names') else [f"f_{j}" for j in range(X_input.shape[1])]):
