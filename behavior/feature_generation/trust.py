...
class TrustScorer:
    ...
    def get_historical_chargeback_rate(customer_profile) -> float:
        if customer_profile.total_transactions == 0:
            return 0.0
        return customer_profile.chargeback_count / max(
            1, customer_profile.total_transactions
        )
    ...
    def get_customer_risk_score(customer_profile) -> float:
        ...
        # Velocity anomalies
        if (
            hasattr(customer_profile, "velocity_stats")
            and customer_profile.velocity_stats.count > 10
        ):
            zscore = customer_profile.velocity_stats.get_zscore(
                customer_profile.velocity_stats.mean
            )
            if zscore > 3:
                score += 0.2
