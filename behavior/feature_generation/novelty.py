...
class NoveltyDetector:
    ...
    @staticmethod
    def get_new_device_flag(customer_profile: CustomerBehaviorProfile, device_id: Optional[str]) -> float:
    ...
    def get_new_country_flag(customer_profile, country_code: Optional[str]) -> float:
        if not country_code:
            return 1.0
        return 0.0 if country_code.upper() in [c.upper() for c in customer_profile.country_frequency.keys()] else 1.0
    ...
    @staticmethod
    def get_new_beneficiary_flag(customer_profile, beneficiary_id: Optional[str]) -> float:
