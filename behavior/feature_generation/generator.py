...
class BehavioralFeatureGenerator:
    ...
    def generate_features(
        ...
        if customer_profile:
            features.update(self._generate_customer_features(transaction, customer_profile))
        
        # Merchant features
        if merchant_profile:
            features.update(self._generate_merchant_features(transaction, merchant_profile))
        ...
        if beneficiary_profile:
            features.update(self._generate_beneficiary_features(transaction, beneficiary_profile))
        
        # Instrument features
        if instrument_profile:
            features.update(self._generate_instrument_features(transaction, instrument_profile))
        
        # Cross-entity similarity features
        features.update(self._generate_cross_entity_features(transaction, **{
    ...
    def _generate_customer_features(self, transaction, profile) -> Dict[str, float]:
        ...
        if hasattr(profile, 'amount_stats') and profile.amount_stats.count > 1:
            features["customer_amount_zscore"] = profile.amount_stats.get_zscore(transaction.amount)
        
        if hasattr(profile, 'amount_ema') and profile.amount_ema:
            features["amount_vs_ema"] = transaction.amount / max(profile.amount_ema.get(), 1)
        ...
        if hasattr(profile, 'velocity_windows'):
            for window_name, window in profile.velocity_windows.items():
                ...
                if window.count > 0:
                    ...
                    features[f"{prefix}_total_amt"] = window.sum if hasattr(window, 'sum') else 0.0
                    features[f"{prefix}_mean_amt"] = window.mean if window.count > 0 else 0.0
        ...
        if hasattr(profile, 'trusted_devices'):
            features["is_new_device"] = 0.0 if transaction.device_id in profile.trusted_devices else 1.0
        
        # Merchant features
        if hasattr(profile, 'merchant_frequency'):
            features["is_new_merchant"] = 0.0 if transaction.merchant_id in profile.merchant_frequency else 1.0
        
        # Country
        if hasattr(profile, 'country_frequency'):
            features["cross_country_flag"] = 0.0 if transaction.country_code in profile.country_frequency else 1.0
        
        # Time features
        features["is_night"] = 1.0 if transaction.timestamp.hour < 6 or transaction.timestamp.hour > 22 else 0.0
    ...
    def _generate_device_features(self, transaction, profile) -> Dict[str, float]:
        ...
        if hasattr(profile, 'device_frequency') and transaction.device_id:
            features["device_account_count"] = float(profile.device_frequency.get(transaction.device_id, 0))
        
        # Also check historical_customers as alternative
        if hasattr(profile, 'historical_customers'):
            features["device_historical_customers"] = float(len(profile.historical_customers))
