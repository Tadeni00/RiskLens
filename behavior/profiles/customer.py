...
class CustomerBehaviorProfile:
    ...
    amount_ema: object = field(
        default_factory=lambda: ExponentialMovingAverage(alpha=0.1)
    )
    ...
    velocity_window: object = field(
        default_factory=lambda: RollingWindow(max_size=1000)
    )
    ...
    def _update_rolling_means(self, amount: float, timestamp: datetime) -> None:
        ...
        self._rolling_mean_7d = self._update_ema(
            self.rolling_mean_7d, self.amount_ema.value, 0.1
        )
        self._rolling_mean_30d = self._update_ema(
            self.rolling_mean_30d, self.amount_ema.value, 0.05
        )
    ...
    def min_amount(self) -> float:
        return (
            min(self.amount_window.get_values())
            if self.amount_window.count > 0
            else 0.0
        )
    ...
    def max_amount(self) -> float:
        return (
            max(self.amount_window.get_values())
            if self.amount_window.count > 0
            else 0.0
        )
    ...
    def amount_zscore(self) -> float:
        return (
            self.amount_stats.get_zscore(self.amount_stats.mean)
            if self.amount_stats.count > 1
            else 0.0
        )
    ...
    def get_merchant_similarity(self, merchant_id: str) -> float:
        ...
        return self.merchant_frequency.get(merchant_id, 0) / max(
            1, sum(self.merchant_frequency.values())
        )
    ...
    def get_customer_amount_zscore(self, amount: float) -> float:
        ...
        return (
            self.amount_stats.get_zscore(amount) if self.amount_stats.count > 1 else 0.0
        )
    ...
    def get_customer_risk_score(self) -> float:
        ...
        if self.total_transactions > 0:
            new_devices = sum(
                1 for v in self.device_fingerprint_frequency.values() if v == 1
            )
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "account_age": (
                (datetime.now(timezone.utc) - self.account_created).days
