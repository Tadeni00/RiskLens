...
LAYOUT_DEFAULTS = dict(
    ...
    font=dict(family="'Inter', 'IBM Plex Sans', sans-serif", color=Colors.TEXT_SECONDARY, size=12),
...
def line_chart(x, y, title: str = "", color: str = None, fill: bool = False,
               height: int = 300, show_grid: bool = True, **kwargs) -> go.Figure:
    ...
    _apply_layout(fig, height=height, title=dict(text=title, font=dict(size=14, color=Colors.TEXT_PRIMARY)))
    if not show_grid:
        fig.update_yaxes(showgrid=False)
    return fig


def area_chart(x, y, title: str = "", color: str = None, height: int = 300,
               **kwargs) -> go.Figure:
    """Create a themed area chart."""
    return line_chart(x, y, title, color, fill=True, height=height, **kwargs)


def bar_chart(x, y, title: str = "", color: str = None, height: int = 300,
              orientation: str = "v", show_values: bool = False, **kwargs) -> go.Figure:
...
def horizontal_bar(categories, values, title: str = "", colors: list = None,
...
def multi_line_chart(data: dict, title: str = "", height: int = 300,
    ...
    if title:
        layout["title"] = dict(text=title, font=dict(size=14, color=Colors.TEXT_PRIMARY))
    if x_label:
...
def gauge_chart(value: float, title: str = "", min_val: float = 0, max_val: float = 1,
                thresholds: list = None, height: int = 250) -> go.Figure:
...
def heatmap_chart(z, x, y, title: str = "", height: int = 350,
...
def scatter_chart(x, y, title: str = "", color: str = None, size: list = None,
                  hover_text: list = None, height: int = 350) -> go.Figure:
...
def waterfall_chart(categories, values, title: str = "", height: int = 350,
                    color_pos: str = None, color_neg: str = None) -> go.Figure:
...
def treemap_chart(labels, parents, values, title: str = "", height: int = 350,
...
def histogram_chart(x, title: str = "", color: str = None, nbins: int = 30,
                    height: int = 300, show_legend: bool = False) -> go.Figure:
...
def dual_axis_chart(x, y1, y2, label1: str = "", label2: str = "",
    ...
    fig.update_layout(
        **LAYOUT_DEFAULTS,
        height=height,
        title=dict(text=title, font=dict(size=14, color=Colors.TEXT_PRIMARY)),
        yaxis=dict(title=label1, titlefont=dict(color=Colors.CHART_1), tickfont=dict(color=Colors.CHART_1)),
        yaxis2=dict(title=label2, titlefont=dict(color=Colors.CHART_2), tickfont=dict(color=Colors.CHART_2), overlaying="y", side="right"),
    )
    return fig


def progress_ring(value: float, max_val: float = 1.0, size: int = 120,
                  color: str = None, label: str = "", show_pct: bool = True):
