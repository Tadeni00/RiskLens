...
def global_header(tenant: str, environment: str = "production",
                  champion_model: str = "CatBoost v1.0",
                  ml_phase: str = "Phase 3", last_refresh: datetime = None):
    """Render the global header bar."""
    refresh_text = last_refresh.strftime("%H:%M:%S") if last_refresh else datetime.now().strftime("%H:%M:%S")
    current_time = datetime.now().strftime("%H:%M:%S UTC")

    phase_class = "phase-3" if "3" in ml_phase else ("phase-2" if "2" in ml_phase else "phase-1")

    env_color = Colors.SUCCESS if environment.lower() == "production" else Colors.WARNING
...
def breadcrumb(items: list):
    ...
    for i, item in enumerate(items):
        if i < len(items) - 1:
            parts.append(f'<span style="color:{Colors.TEXT_MUTED};cursor:pointer">{item}</span>')
        else:
            parts.append(f'<span style="color:{Colors.TEXT_PRIMARY};font-weight:{Typography.WEIGHT_MEDIUM}">{item}</span>')
    st.markdown(
