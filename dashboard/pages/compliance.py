    ...
    with page_container(
        ...
        if not df.empty and "is_fraud" in df.columns and "decision" in df.columns:
            ...
            with col_geo:
                ...
                if "country_code" in df_work.columns:
                    geo = (
                        df_work.groupby("country_code")["is_flagged"]
                        .agg(["mean", "count"])
                        .reset_index()
                    )
                    geo = geo[geo["count"] > 5]
                ...
                if geo_data:
                    ...
                    fig_geo.update_layout(
                        ...
                        title=dict(
                            text="Geographic Region (80% Rule)",
            ...
            with col_ch:
                ...
                if "channel" in df_work.columns:
                    ch = (
                        df_work.groupby("channel")["is_flagged"]
                        .agg(["mean", "count"])
                        .reset_index()
                    )
                    ch = ch[ch["count"] > 5]
                ...
                if ch_data:
                    ...
                    fig_ch.update_layout(
                        ...
                        xaxis=dict(tickfont=dict(color=Colors.TEXT_MUTED, size=11)),
                        title=dict(
                            text="Transaction Channel (80% Rule)",
                            font=dict(size=13, color=Colors.TEXT_PRIMARY),
                        ),
                    )
        ...
        for i in range(n_audit):
            ...
            decision = (
                ...
                else "REVIEW" if "REVIEW" in event_type else rng.choice(decisions)
            )
            ...
            [
                ...
                },
                {
                    ...
                    "color": (
                        Colors.CRITICAL
                        if p99_lat > 120
                        else Colors.WARNING if p99_lat > 90 else Colors.SUCCESS
                    ),
                    "icon": "ZAP",
                },
                {
