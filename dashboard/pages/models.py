...
from dashboard.components.data_loader import make_pr_curve, make_confusion_matrix, make_feature_importance
...
MODEL_LEADERBOARD = pd.DataFrame([
    {"Rank": 1, "Model": "CatBoost", "Version": "v3.2.1", "PR-AUC": 0.9347, "ROC-AUC": 0.9891,
     "F2": 0.8712, "Latency (ms)": 12.4, "Status": "Champion"},
    {"Rank": 2, "Model": "FT-Transformer", "Version": "v1.0.0", "PR-AUC": 0.9218, "ROC-AUC": 0.9845,
     "F2": 0.8534, "Latency (ms)": 48.7, "Status": "Specialist"},
    {"Rank": 3, "Model": "LightGBM", "Version": "v4.1.0", "PR-AUC": 0.9183, "ROC-AUC": 0.9812,
     "F2": 0.8401, "Latency (ms)": 8.2, "Status": "Offline"},
    {"Rank": 4, "Model": "XGBoost", "Version": "v2.0.3", "PR-AUC": 0.9097, "ROC-AUC": 0.9788,
     "F2": 0.8325, "Latency (ms)": 9.6, "Status": "Offline"},
])
...
def _generate_training_history():
    ...
    pr_auc = 0.5 + 0.44 * (1 - np.exp(-iterations / 80)) + rng.normal(0, 0.008, 500).cumsum() * 0.01
    pr_auc = np.clip(pr_auc, 0.5, 0.94)
    pr_auc[-1] = 0.9347

    loss = 0.7 * np.exp(-iterations / 60) + 0.15 + rng.normal(0, 0.005, 500).cumsum() * 0.005
...
def render(tenant_id: str):
    ...
    with page_container("Model Architecture", "Production ML models and performance analysis", "BRAIN"):

        # ── KPI Strip ─────────────────────────────────────────────────────────
        kpi_row([
            {"label": "PR-AUC", "value": f"{CHAMPION_METRICS['PR-AUC']:.4f}", "icon": "TARGET", "status": "healthy"},
            {"label": "ROC-AUC", "value": f"{CHAMPION_METRICS['ROC-AUC']:.4f}", "icon": "ACTIVITY", "status": "healthy"},
            {"label": "F2-Score", "value": f"{CHAMPION_METRICS['F2-Score']:.4f}", "icon": "TRENDING_UP", "status": "healthy"},
            {"label": "Latency", "value": f"{CHAMPION_METRICS['Latency (ms)']:.1f}ms", "icon": "TIMER", "status": "healthy"},
            {"label": "Precision", "value": f"{CHAMPION_METRICS['Precision']:.4f}", "icon": "CROSSHAIR"},
            {"label": "Recall", "value": f"{CHAMPION_METRICS['Recall']:.4f}", "icon": "SEARCH"},
        ])
        ...
        prob_true = np.array([0.002, 0.008, 0.018, 0.035, 0.052, 0.098, 0.145, 0.230, 0.380, 0.560])
        prob_pred = np.array([0.005, 0.012, 0.025, 0.040, 0.065, 0.105, 0.160, 0.250, 0.400, 0.580])
        ...
        fig_cal.add_trace(go.Scatter(
            ...
            marker=dict(size=8, color=Colors.ACCENT, line=dict(width=1.5, color=Colors.BG_CARD)),
        ))
        fig_cal.update_layout(
            ...
            xaxis=dict(range=[-0.02, 1.02], showgrid=True, gridcolor=Colors.BORDER_SUBTLE),
            yaxis=dict(range=[-0.02, 1.02], showgrid=True, gridcolor=Colors.BORDER_SUBTLE),
            legend=dict(
        ...
        fi_colors = [
            Colors.CRITICAL if v > 0.12
            else Colors.WARNING if v > 0.06
            else Colors.ACCENT
        ...
        with col_metrics:
            metric_row([
                {"label": "Accuracy", "value": f"{accuracy:.4f}", "icon": "CHECK_CIRCLE"},
                {"label": "Precision", "value": f"{precision:.4f}", "icon": "CROSSHAIR"},
            ])
            st.markdown("<div style='height:8px'></div>", unsafe_allow_html=True)
            metric_row([
                {"label": "Recall", "value": f"{recall:.4f}", "icon": "SEARCH"},
                {"label": "Total Samples", "value": f"{total:,}", "icon": "BAR_CHART"},
            ])
            st.markdown("<div style='height:16px'></div>", unsafe_allow_html=True)

            metric_table(
                metrics=[
                    {"label": "True Positives", "value": f"{tp:,}", "status": "healthy"},
                    {"label": "False Positives", "value": f"{fp:,}", "status": "warning"},
                    {"label": "True Negatives", "value": f"{tn:,}", "status": "healthy"},
                    {"label": "False Negatives", "value": f"{fn:,}", "status": "critical"},
                ],
        ...
        with col_pr:
            ...
            fig_pr_hist.add_hline(
                ...
                line_dash="dash", line_color=Colors.SUCCESS, line_width=1.5,
            ...
            fig_pr_hist.update_layout(
                title=dict(text="PR-AUC Over Training", font=dict(size=14, color=Colors.TEXT_PRIMARY)),
        ...
        with col_loss:
            ...
            fig_loss.add_hline(
                y=train_loss[-1],
                line_dash="dash", line_color=Colors.CHART_4, line_width=1.5,
                annotation_text=f"Final: {train_loss[-1]:.4f}",
                annotation_position="top right",
                annotation_font=dict(color=Colors.CHART_4, size=11),
            )
            fig_loss.update_layout(
                title=dict(text="Loss Over Training", font=dict(size=14, color=Colors.TEXT_PRIMARY)),
                xaxis_title="Iteration",
                yaxis_title="Loss",
                height=320,
                paper_bgcolor="rgba(0,0,0,0)",
