...
def render(tenant_id: str):
    ...
    st.caption("Live metrics evaluated on recent scored transactions from the streaming API.")
    ...
    if total_pos == 0:
        st.warning("No fraud labels in the recent data stream. Waiting for more data to compute curves.")
        return
    ...
    col2.metric("F2-Score",    f"{(5*precision_val*recall_val)/(4*precision_val+recall_val+1e-8):.3f}")
    ...
    with col_l:
        ...
        fig_pr.add_trace(go.Scatter(
            ...
            mode="lines", name=f"Live (PR-AUC={pr_auc:.3f})",
            line=dict(color="#3B82F6", width=2.5),
            fill="tozeroy", fillcolor="rgba(59,130,246,0.1)",
        ))
        fig_pr.add_hline(
            y=fraud_rate, line_dash="dash", line_color="#888",
    ...
    fig_dist.add_trace(go.Histogram(
        x=legit_scores, name="Legitimate", nbinsx=50,
        marker_color="#22C55E", opacity=0.6, histnorm="probability density",
    ))
    fig_dist.add_trace(go.Histogram(
        x=fraud_scores, name="Fraud", nbinsx=50,
        marker_color="#EF4444", opacity=0.7, histnorm="probability density",
    ))
    ...
    fig_cal.add_trace(go.Scatter(
        ...
        name="Perfect calibration", line=dict(color="#888", dash="dash"),
    ))
    fig_cal.add_trace(go.Scatter(
        x=prob_pred, y=prob_true, mode="lines+markers",
        name="Live Model", line=dict(color="#3B82F6", width=2.5),
        marker=dict(size=7),
    ))
    fig_cal.update_layout(
        xaxis_title="Mean Predicted Score", yaxis_title="Fraction of Positives",
    ...
    st.caption("Absolute correlation of features with fraud labels in the recent stream.")
    ...
    colors = ["#EF4444" if imp > 0.3 else "#F59E0B" if imp > 0.15 else "#3B82F6"
              for imp in fi_sorted["importance"]]
    fig_fi = go.Figure(go.Bar(
        x=fi_sorted["importance"], y=fi_sorted["feature"],
        orientation="h", marker_color=colors,
        text=fi_sorted["importance"].round(3), textposition="outside",
    ))
