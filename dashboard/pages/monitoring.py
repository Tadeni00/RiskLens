...
def render(tenant_id: str):
    ...
    with page_container("Live Monitoring", "Real-time system health and performance metrics", "ACTIVITY"):
        ...
        fig_lat.add_trace(go.Scatter(
            x=live["timestamp"], y=live["latency_p95"],
            mode="lines", fill="tozeroy",
            line=dict(color=Colors.CHART_1, width=2, shape="spline"),
            fillcolor=Colors.rgba(Colors.CHART_1, 0.12),
            name="P95 Latency",
        ))
        fig_lat.add_hline(y=200, line_dash="dash", line_color=Colors.CRITICAL,
                          annotation_text="Critical (200ms)", annotation_font_color=Colors.CRITICAL)
        fig_lat.add_hline(y=100, line_dash="dash", line_color=Colors.WARNING,
                          annotation_text="Warning (100ms)", annotation_font_color=Colors.WARNING)
        fig_lat.update_layout(
            height=280, paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
            font=dict(family="'Inter', sans-serif", color=Colors.TEXT_SECONDARY, size=12),
            margin=dict(l=0, r=0, t=20, b=0),
            yaxis=dict(title="ms", showgrid=True, gridcolor=Colors.BORDER_SUBTLE,
                       tickfont=dict(color=Colors.TEXT_MUTED, size=11)),
            xaxis=dict(showgrid=False, tickfont=dict(color=Colors.TEXT_MUTED, size=11)),
            hoverlabel=dict(bgcolor=Colors.BG_ELEVATED, bordercolor=Colors.BORDER_DEFAULT,
        ...
        with col_tps:
            ...
            fig_tps.update_layout(
                height=280, paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
                font=dict(family="'Inter', sans-serif", color=Colors.TEXT_SECONDARY, size=12),
                title=dict(text="Transactions per Second", font=dict(size=14, color=Colors.TEXT_PRIMARY)),
                margin=dict(l=0, r=0, t=32, b=0),
                yaxis=dict(title="txns/sec", showgrid=True, gridcolor=Colors.BORDER_SUBTLE,
                           tickfont=dict(color=Colors.TEXT_MUTED, size=11)),
                xaxis=dict(showgrid=False, tickfont=dict(color=Colors.TEXT_MUTED, size=11)),
                hoverlabel=dict(bgcolor=Colors.BG_ELEVATED, bordercolor=Colors.BORDER_DEFAULT,
                                font=dict(color=Colors.TEXT_PRIMARY, size=12)),
        ...
        with col_pred:
            ...
            fig_pred.add_trace(go.Scatter(
                x=live["timestamp"], y=pred_throughput,
                mode="lines", fill="tozeroy",
            ...
            fig_pred.update_layout(
                height=280, paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
                font=dict(family="'Inter', sans-serif", color=Colors.TEXT_SECONDARY, size=12),
                title=dict(text="Prediction Throughput", font=dict(size=14, color=Colors.TEXT_PRIMARY)),
                margin=dict(l=0, r=0, t=32, b=0),
                yaxis=dict(title="preds/sec", showgrid=True, gridcolor=Colors.BORDER_SUBTLE,
                           tickfont=dict(color=Colors.TEXT_MUTED, size=11)),
                xaxis=dict(showgrid=False, tickfont=dict(color=Colors.TEXT_MUTED, size=11)),
                hoverlabel=dict(bgcolor=Colors.BG_ELEVATED, bordercolor=Colors.BORDER_DEFAULT,
                                font=dict(color=Colors.TEXT_PRIMARY, size=12)),
        ...
        infra = [
            {
                ...
                "icon": "CPU",
                "metrics": {"Current": f"{rng.integers(42, 68)}%", "Load Avg": f"{rng.uniform(1.2, 3.8):.1f}", "Cores": "16"},
            },
            {
                "title": "Memory Usage",
                "status": "healthy",
                ...
                "icon": "SERVER",
                "metrics": {"Used": f"{rng.uniform(38, 52):.1f}GB", "Cached": f"{rng.uniform(4, 8):.1f}GB", "Swap": "0.2GB"},
            },
            {
                ...
                "icon": "DATABASE",
                "metrics": {"Hit Rate": f"{rng.uniform(93, 98):.1f}%", "Memory": "2.1GB", "Evictions": "0"},
            },
            {
                "title": "Kafka Lag",
                "status": "warning",
                ...
                "icon": "Zap",
                "metrics": {"Lag": f"{rng.integers(8000, 15000):,}", "Throughput": f"{rng.uniform(7, 12):.1f}K/s", "Partitions": "24"},
            },
            {
                ...
                "icon": "DATABASE",
                "metrics": {"Active": f"{rng.integers(18, 32)}", "Idle": f"{rng.integers(5, 12)}", "Max": "100"},
            },
            {
                ...
                "icon": "CPU",
                "metrics": {"Util": f"{rng.integers(55, 82)}%", "VRAM": f"{rng.integers(14, 22)}/24GB", "Temp": f"{rng.integers(58, 74)}°C"},
            },
        ]
        ...
        fig_err.add_trace(go.Scatter(
            x=live["timestamp"], y=error_rates,
            mode="lines", fill="tozeroy",
            line=dict(color=Colors.CHART_4, width=2, shape="spline"),
            fillcolor=Colors.rgba(Colors.CHART_4, 0.12),
            name="Error Rate",
        ))
        fig_err.add_hline(y=0.5, line_dash="dash", line_color=Colors.WARNING,
                          annotation_text="Threshold (0.5%)", annotation_font_color=Colors.WARNING)
        fig_err.update_layout(
            height=280, paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
            font=dict(family="'Inter', sans-serif", color=Colors.TEXT_SECONDARY, size=12),
            title=dict(text="Error Rate Over Time", font=dict(size=14, color=Colors.TEXT_PRIMARY)),
            margin=dict(l=0, r=0, t=32, b=0),
            yaxis=dict(title="%", showgrid=True, gridcolor=Colors.BORDER_SUBTLE,
                       tickfont=dict(color=Colors.TEXT_MUTED, size=11)),
            xaxis=dict(showgrid=False, tickfont=dict(color=Colors.TEXT_MUTED, size=11)),
            hoverlabel=dict(bgcolor=Colors.BG_ELEVATED, bordercolor=Colors.BORDER_DEFAULT,
                            font=dict(color=Colors.TEXT_PRIMARY, size=12)),
        ...
        error_types = ["Timeout", "Connection Refused", "Rate Limited", "Validation Error", "Internal Error"]
        error_counts = rng.poisson([12, 8, 5, 3, 2]).tolist()

        fig_err_dist = go.Figure()
        fig_err_dist.add_trace(go.Bar(
            x=error_types, y=error_counts,
            marker=dict(
                color=[Colors.CRITICAL, Colors.WARNING, Colors.ACCENT, Colors.CHART_5, Colors.CHART_4],
                cornerradius=4,
            ),
            text=error_counts,
            textposition="auto",
        ...
        fig_err_dist.update_layout(
            height=280, paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
            font=dict(family="'Inter', sans-serif", color=Colors.TEXT_SECONDARY, size=12),
            title=dict(text="Error Distribution by Type", font=dict(size=14, color=Colors.TEXT_PRIMARY)),
            margin=dict(l=0, r=0, t=32, b=0),
            yaxis=dict(title="Count", showgrid=True, gridcolor=Colors.BORDER_SUBTLE,
                       tickfont=dict(color=Colors.TEXT_MUTED, size=11)),
            xaxis=dict(showgrid=False, tickfont=dict(color=Colors.TEXT_MUTED, size=11)),
            hoverlabel=dict(bgcolor=Colors.BG_ELEVATED, bordercolor=Colors.BORDER_DEFAULT,
                            font=dict(color=Colors.TEXT_PRIMARY, size=12)),
