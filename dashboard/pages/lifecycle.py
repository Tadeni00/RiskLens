...
from dashboard.components import (
    kpi_row, bar_chart, line_chart, gauge_chart, data_table,
    metric_table, page_container, section_divider, lifecycle_timeline,
    model_performance_summary, metric_row, progress_ring,
)
...
def _generate_synthetic_data():
    ...
    # Model registry
    models = [
        {"Model": "CatBoost", "Version": "3.2.1", "Status": "Champion",
         "PR-AUC": 0.8342, "Latency": "42ms", "Training Date": "2026-07-10", "Actions": "View"},
        {"Model": "FT-Transformer", "Version": "2.1.0", "Status": "Specialist",
         "PR-AUC": 0.8127, "Latency": "68ms", "Training Date": "2026-07-08", "Actions": "View"},
        {"Model": "NetPFN", "Version": "1.4.3", "Status": "Specialist",
         "PR-AUC": 0.7654, "Latency": "31ms", "Training Date": "2026-07-05", "Actions": "View"},
        {"Model": "Isolation Forest", "Version": "2.0.0", "Status": "Offline",
         "PR-AUC": 0.6891, "Latency": "18ms", "Training Date": "2026-06-28", "Actions": "View"},
        {"Model": "VAE Anomaly", "Version": "1.1.2", "Status": "Offline",
         "PR-AUC": 0.6543, "Latency": "55ms", "Training Date": "2026-06-20", "Actions": "View"},
        {"Model": "CatBoost v2", "Version": "3.1.0", "Status": "Archived",
         "PR-AUC": 0.7988, "Latency": "44ms", "Training Date": "2026-06-15", "Actions": "View"},
    ]
...
def render(tenant_id: str):
    ...
    with page_container("Model Lifecycle", "Model registry, promotion workflow, and lifecycle management", "LAYERS"):

        # ── KPI Row ───────────────────────────────────────────────────────────
        kpi_row([
            {"label": "Total Scored", "value": f"{data['total_scored']:,}", "icon": "DATABASE"},
            {"label": "Active Models", "value": str(data['active_models']), "icon": "BOX"},
            {"label": "Champion PR-AUC", "value": f"{data['pr_auc']:.4f}", "icon": "TARGET"},
            {"label": "P95 Latency", "value": f"{data['latency_ms']:.0f}ms", "icon": "TIMER"},
        ])
...
""", unsafe_allow_html=True)
        ...
        for i, (col, phase) in enumerate(zip(phase_cols, phases)):
            progress_pct = min(phase["current"] / phase["target"] * 100, 100) if phase["target"] > 0 else 100
...
""", unsafe_allow_html=True)
        ...
        with col_left:
            model_performance_summary("CatBoost v3.2.1", {
                "PR-AUC": 0.8342,
                "ROC-AUC": 0.9123,
                "F2-Score": 0.7891,
                "Latency": "42.3ms",
        ...
        with col_right:
            st.plotly_chart(gauge_chart(
                ...
                max_val=1,
                thresholds=[
                    {"range": [0, 0.50], "color": Colors.rgba(Colors.CRITICAL, 0.3)},
                    {"range": [0.50, 0.78], "color": Colors.rgba(Colors.WARNING, 0.3)},
                    {"range": [0.78, 1.0], "color": Colors.rgba(Colors.SUCCESS, 0.3)},
                ],
                height=280,
...
""", unsafe_allow_html=True)
        ...
        for _, row in registry_df.iterrows():
            ...
            badge_bg = Colors.SUCCESS if row["Status"] == "Champion" else \
                       Colors.INFO if row["Status"] == "Specialist" else \
                       Colors.WARNING if row["Status"] == "Offline" else \
                       Colors.TEXT_MUTED
...
""", unsafe_allow_html=True)
        ...
        with vol_col:
            ...
            fig_vol.update_layout(
                ...
                hovermode="x unified",
                title=dict(text="Scoring Volume Over Time", font=dict(size=14, color=Colors.TEXT_PRIMARY)),
            )
        ...
        with risk_col:
            ...
            fig_risk.add_hline(
                y=0.40, line_dash="dash", line_color=Colors.WARNING,
                annotation_text="Review threshold (0.40)",
                annotation_position="top right",
                annotation_font=dict(color=Colors.WARNING, size=10),
            )
            fig_risk.add_hline(
                y=0.85, line_dash="dash", line_color=Colors.CRITICAL,
            ...
            fig_risk.update_layout(
                ...
                yaxis=dict(
                    ...
                    showline=False, zeroline=False,
                    tickfont=dict(color=Colors.TEXT_MUTED, size=10),
                    title=dict(text="Avg Risk Score", font=dict(color=Colors.TEXT_MUTED, size=11)),
                    range=[0, 1.0],
                ),
                legend=dict(
                    font=dict(color=Colors.TEXT_SECONDARY, size=11),
                ...
                hovermode="x unified",
                title=dict(text="Average Risk Score Trend", font=dict(size=14, color=Colors.TEXT_PRIMARY)),
            )
