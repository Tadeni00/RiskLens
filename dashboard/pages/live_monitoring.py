...
def render(tenant_id: str):
    ...
    head_6 = live["fraud_rate"].head(6).mean() if len(live) >= 6 else live["fraud_rate"].mean()
    tail_6 = live["fraud_rate"].tail(6).mean() if len(live) >= 6 else live["fraud_rate"].mean()

    c1, c2, c3, c4 = st.columns(4)
    c1.metric("Txn/min (now)",   f"{int(last_row['txn_per_min']):,}")
    c2.metric("Fraud Rate (1h)", f"{tail_6*100:.2f}%",
              delta=f"{(tail_6 - head_6)*100:+.2f}%")
    c3.metric("P95 Latency",     f"{last_row['latency_p95']:.0f}ms",
              delta_color="inverse", delta=f"{last_row['latency_p95']-78:.0f}ms vs target")
    ...
    fig.add_hline(y=90,  line_dash="dot",  line_color="#F59E0B",
                  annotation_text="Warning (90ms)")
    fig.update_layout(
        height=280, paper_bgcolor="rgba(0,0,0,0)",
        plot_bgcolor="rgba(0,0,0,0)",
        yaxis_title="ms", margin=dict(l=0, r=0, t=20, b=0),
    )
    ...
    fig2.update_layout(
        height=250, yaxis_title="Fraud Rate (%)",
