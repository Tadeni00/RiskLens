...
def render(tenant_id: str):
    ...
    st.caption(
        "Exploratory analysis of the raw transaction dataset ingested for this tenant."
    )
    ...
    st.caption(
        "Select a feature to compare its distribution between legitimate and fraud transactions."
    )
    ...
    with col_a:
        ...
        s = (
            pd.Series(legit_vals).describe().round(3)
            if legit_vals
    ...
    with col_b:
        st.markdown("**Fraud**")
        s = (
            pd.Series(fraud_vals).describe().round(3)
            if fraud_vals
            else pd.Series(dtype=float)
        )
        st.dataframe(s.to_frame("value"), use_container_width=True)
    ...
    st.caption(
        "Strong correlations between features may indicate redundancy. Highlighted cells show |r| > 0.5."
    )
