...
def _make_drift_history() -> pd.DataFrame:
    ...
    for i in range(20):
        ...
        if psi < 0.10:
            ...
            status = "Critical"
            action = rng.choice(
                ["Retrain Scheduled", "Retraining Complete", "Auto-Retrained"]
            )
...
def render(tenant_id: str):
    ...
    with page_container(
            ...
            [
                {
                    ...
                    "Status": (
                        "Critical"
                        if v["psi"] > 0.20
                        else "Warning" if v["psi"] > 0.10 else "Stable"
                    ),
                }
        ...
        status_color = (
            "critical"
            if d["psi"] > 0.20
            else "warning" if d["psi"] > 0.10 else "healthy"
        )
        st.markdown(
        ...
        with col_kl:
            ...
            fig_kl.update_layout(
                height=280,
                title=dict(
                    text="Prediction Drift (KL Divergence)",
        ...
        with col_pr:
            ...
            fig_pr.update_layout(
                height=280,
                title=dict(
                    text="Model Performance Drift (PR-AUC)",
                    font=dict(size=14, color=Colors.TEXT_PRIMARY),
                ),
                paper_bgcolor="rgba(0,0,0,0)",
                plot_bgcolor="rgba(0,0,0,0)",
                ...
                yaxis=dict(
                    title=dict(
                        text="PR-AUC", font=dict(size=12, color=Colors.TEXT_MUTED)
                    ),
                    showgrid=True,
                    gridcolor=Colors.BORDER_SUBTLE,
                    zeroline=False,
        ...
        metric_row(
            [
                ...
                },
                {
                    ...
                    "value": f"{latest_pr_auc:.4f}",
                    "color": (
                        Colors.SUCCESS if latest_pr_auc > 0.80 else Colors.CRITICAL
                    ),
                    "icon": "TARGET",
                },
                {
        ...
        for feat in warning_features:
            ...
            alert(
                ...
                - timedelta(minutes=int(psi_val * 100)),
            )
        ...
        n_retrains = len(
            history_df[history_df["Action Taken"].str.contains("Retrain", case=False)]
        )
        ...
        metric_row(
            [
                ...
                },
                {
                    ...
                    "value": str(n_retrains),
                    "color": Colors.WARNING if n_retrains > 0 else Colors.SUCCESS,
                    "icon": "REFRESH",
                },
                {
                    "label": "Avg Historical PSI",
                    "value": f"{avg_psi_history:.4f}",
                    "color": (
                        Colors.WARNING if avg_psi_history > 0.15 else Colors.SUCCESS
                    ),
                    "icon": "BAR_CHART",
                },
            ]
