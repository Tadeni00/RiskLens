"""FraudTrap Dashboard — API Integration Tests (pytest-compatible)"""
import pytest
import requests
...
@pytest.mark.skipif(not _is_reachable(), reason="API server not running on localhost:8000")
@pytest.mark.parametrize("desc,method,endpoint", TESTS, ids=[t[0] for t in TESTS])
def test_api_endpoint(desc, method, endpoint):
    r = requests.request(method, f"{BASE}{endpoint}", timeout=5)
    assert r.status_code == 200, f"{desc} returned {r.status_code}: {r.text[:200]}"

    data = r.json()
    assert isinstance(data, (dict, list)), f"{desc} returned unexpected type: {type(data)}"
