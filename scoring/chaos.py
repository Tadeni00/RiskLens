...
class ChaosMonkey:
    ...
    def inject_model_missing(self, tenant_id: str, duration: int = 60) -> str:
        ...
        logger.warning("CHAOS: Model missing for tenant={} for {}s", tenant_id, duration)
        return exp_id
    
    def inject_latency(self, min_ms: int = 50, max_ms: int = 200, duration: int = 60) -> str:
        """Inject random latency into scoring path."""
        exp_id = f"latency_{int(time.time())}"
        logger.warning("CHAOS: Latency injection {}-{}ms for {}s", min_ms, max_ms, duration)
        return exp_id
    ...
    def chaos(self, experiment: ChaosExperiment):
        ...
        try:
            if experiment.chaos_type == ChaosType.REDIS_DOWN:
                ...
                self.inject_model_missing(experiment.target_service or "all", experiment.duration_seconds)
...
if __name__ == "__main__":
    ...
    with monkey.chaos(ChaosExperiment(
