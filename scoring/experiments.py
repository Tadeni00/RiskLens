...
class ExperimentConfig:
    ...
    # Safety
    guardrail_metrics: list[str] = field(default_factory=lambda: ["error_rate", "latency_p95"])
    guardrail_thresholds: dict[str, float] = field(default_factory=lambda: {"error_rate": 0.01, "latency_p95": 200})
...
class ExperimentRouter:
    ...
    def route(self, traffic_key: str) -> str:
        ...
        champion = next(v for v in self.experiment.variants if v.variant_type == VariantType.CHAMPION)
        return champion.name
    
    def get_variant_model_id(self, variant_name: str) -> Optional[str]:
        """Get model ID for a variant."""
        variant = next((v for v in self.experiment.variants if v.name == variant_name), None)
...
class ExperimentManager:
    ...
    def _validate_experiment(self, config: ExperimentConfig) -> None:
        ...
        champions = [v for v in config.variants if v.variant_type == VariantType.CHAMPION]
        if len(champions) != 1:
    ...
    def start_experiment(self, name: str) -> bool:
        ...
        with self._lock:
            ...
            if exp.status != ExperimentStatus.DRAFT:
                logger.warning("Cannot start experiment {} in status {}", name, exp.status)
                return False
    ...
    def get_variant_model(self, experiment_name: str, traffic_key: str) -> tuple[Optional[str], Optional[str]]:
    ...
    def record_outcome(
        ...
        with self._lock:
            ...
            if result.samples == 1:
                ...
                # Running mean
                result.metric_value = ((result.samples - 1) * result.metric_value + metric_value) / result.samples
            
            # Check guardrails
            if guardrails:
                violations = []
                for metric, threshold in self._experiments[self.experiment_name].guardrail_thresholds.items():
                    if metric in guardrails and guardrails[metric] > threshold:
                        violations.append(f"{metric}: {guardrails[metric]} > {threshold}")
    ...
    def compute_statistics(self, experiment_name: str) -> dict[str, ExperimentResult]:
        ...
        with self._lock:
            ...
            # Find champion
            champion_name = next(v.name for v in exp.variants if v.variant_type == VariantType.CHAMPION)
            ...
            for variant_name, result in results.items():
                ...
                try:
                    ...
                    if exp.primary_metric in ("conversion_rate", "fraud_capture_rate"):
                        ...
                        p1 = champion_result.conversions / max(champion_result.samples, 1)
                        p2 = result.conversions / max(result.samples, 1)
                        pooled_p = (champion_result.conversions + result.conversions) / (champion_result.samples + result.samples)
                        se = np.sqrt(pooled_p * (1 - pooled_p) * (1/champion_result.samples + 1/result.samples))
                        if se > 0:
                            z = (p2 - p1) / se
                    ...
                    if result.samples > 0:
                        se = np.sqrt(result.metric_value * (1 - result.metric_value) / result.samples)
                    ...
                    logger.warning("Statistical test failed for {}: {}", experiment_name, exc)
    ...
    def should_stop_early(self, experiment_name: str) -> tuple[bool, str]:
        ...
        champion = next((r for r in stats.values() if r.variant_type == VariantType.CHAMPION), None)
        challengers = [r for r in stats.values() if r.variant_type == VariantType.CHALLENGER]
        
        for challenger in challengers:
            if challenger.is_significant:
                lift = challenger.lift_pct
                if lift > 0:
                    return True, f"Challenger {challenger.variant_name} significantly better (lift: {lift:.1f}%)"
                else:
                    return True, f"Challenger {challenger.variant_name} significantly worse"
