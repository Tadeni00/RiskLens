...
class SafeExpressionEvaluator:
    ...
    _ALLOWED_NODES = {
        'Expression', 'Expr', 'Compare', 'BoolOp', 'BinOp', 'UnaryOp',
        'Name', 'Constant', 'Load',
        'And', 'Or', 'Not',
    ...
    def evaluate(self, expression: str, features: dict[str, float]) -> bool:
        ...
        try:
            ...
            logger.warning("Expression evaluation failed: {} | expr={}", exc, expression)
            return False
...
class RulesEngine:
    ...
    def reload_rules(self) -> None:
        ...
        if self._rules_path:
            try:
                ...
                logger.info("Loaded {} rules from {}", len(self._rules), self._rules_path)
                return
            except Exception as exc:
        ...
        if self._redis:
            try:
                ...
                if raw:
                    data = json.loads(raw)
                    self._rules = [RuleDefinition.from_dict(r) for r in data.get("rules", [])]
    ...
    def save_rules(self, rules: list[RuleDefinition]) -> None:
        ...
        data = {"rules": [r.to_dict() for r in rules], "updated_at": datetime.now(timezone.utc).isoformat()}
    ...
    def evaluate(self, txn: TransactionRequest, features: dict[str, float]) -> RuleResult:
        ...
        for rule in self._rules:
            ...
            try:
                rule_triggered, contribution, is_hard_block, override = self._evaluate_rule(rule, txn, features)
    ...
    def _check_blocklist(self, config: BlocklistConfig, txn: TransactionRequest) -> bool:
        ...
        try:
            return bool(self._redis.sismember(f"ft:blocklist:{config.list_name}", value))
        except Exception:
            return False
    
    def _check_threshold(self, config: ThresholdConfig, features: dict[str, float]) -> bool:
        val = features.get(config.feature, 0.0)
        return self._compare(val, config.operator, config.threshold)
    
    def _check_velocity(self, config: VelocityConfig, features: dict[str, float]) -> bool:
        feature_map = {
            "account": "acct",
            "device": "dev",
            "ip": "ip",
    ...
    def _check_geo(self, config: GeoConfig, txn: TransactionRequest, features: dict[str, float]) -> bool:
    ...
    def explain(self, txn: TransactionRequest, features: dict[str, float]) -> dict:
        ...
        return {
            "model_type": "rules",
            "base_value": 0.0,
            "prediction_value": min(1.0, result.risk_boost + (1.0 if result.hard_block else 0.0)),
            "top_features": [
                {
                    "feature": c.rule_id,
...
class BlocklistManager:
    ...
    def add(
        ...
        pipe.hset(self._entry_key(tenant_id, list_name, value), mapping=entry.to_redis_hash())
        ...
        logger.info("Blocklist add: tenant={} list={} value={} by={}", 
                   tenant_id, list_name, value, added_by)
        return entry
    
    def remove(self, list_name: str, value: str, tenant_id: str, removed_by: str, reason: str) -> bool:
        ...
        if removed:
            ...
            logger.info("Blocklist remove: tenant={} list={} value={} by={}", 
                       tenant_id, list_name, value, removed_by)
        return bool(removed)
    
    def get_entry(self, list_name: str, value: str, tenant_id: str) -> Optional[BlocklistEntry]:
    ...
    def list_entries(self, list_name: str, tenant_id: str, active_only: bool = True) -> list[BlocklistEntry]:
...
class HeuristicConfig:
    ...
    components: dict[str, dict] = field(default_factory=lambda: {
        ...
        "high_risk_channel": {"weight": 0.05, "transform": "x in [2, 5]", "cap": 0.05},
    })
    max_score: float = 1.0
