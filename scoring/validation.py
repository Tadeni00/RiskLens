...
def validate_feature_compatibility(tenant_id: str, features: dict[str, float]) -> bool:
    ...
    try:
        ...
        if schema is None:
            ...
            logger.debug("No schema registered for tenant={}, allowing request", tenant_id)
            return True
        ...
        has_mismatch = (live_hash != schema.feature_hash) or bool(missing) or bool(extra)
        
        if has_mismatch:
            logger.warning(
                "Feature schema mismatch for tenant={}: live_hash={} registered_hash={} "
                "missing={} extra={}",
                tenant_id, live_hash, schema.feature_hash, missing[:10], extra[:10]
            )
