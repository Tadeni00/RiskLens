...
class FeatureStoreClient(ABC):
    ...
    @abstractmethod
    def set_home_country(self, tenant_id: str, account_id: str, country_code: str) -> None:
    ...
    @abstractmethod
    def add_device_account(self, tenant_id: str, device_id: str, account_id: str) -> None:
...
class RedisFeatureStore(FeatureStoreClient):
    ...
    def get_velocity_features(
        ...
        try:
            ...
            for i, window_sec in enumerate(windows):
                ...
                if window_sec == 60:
                    features.count_1m, features.total_amt_1m, features.mean_amt_1m = count, total_amt, mean_amt
                elif window_sec == 300:
                    features.count_5m, features.total_amt_5m, features.mean_amt_5m = count, total_amt, mean_amt
                elif window_sec == 3600:
                    features.count_1h, features.total_amt_1h, features.mean_amt_1h = count, total_amt, mean_amt
                elif window_sec == 86400:
                    features.count_24h, features.total_amt_24h, features.mean_amt_24h = count, total_amt, mean_amt
                elif window_sec == 604800:
                    features.count_7d, features.total_amt_7d, features.mean_amt_7d = count, total_amt, mean_amt
    ...
    def update_hist_stats(self, tenant_id: str, account_id: str, amount: float) -> None:
        ...
        try:
            ...
            new_std = np.sqrt(
                ((count - 1) * old_std**2 + (amount - old_mean) * (amount - new_mean)) / count
            ) if count > 1 else 1.0
    ...
    def update_typing_baseline(
        ...
        try:
            ...
            new_std = np.sqrt(
                ((n - 1) * old_std**2 + (cadence_ms - old_mean) * (cadence_ms - new_mean)) / n
            ) if n > 1 else 50.0
...
class InMemoryFeatureStore(FeatureStoreClient):
    ...
    def get_velocity_features(self, entity_type: str, entity_id: str, tenant_id: str, windows_seconds: list[int] = None) -> VelocityFeatures:
        return VelocityFeatures()
    
    def increment_velocity(self, entity_type: str, entity_id: str, tenant_id: str, amount: float, transaction_id: str, timestamp: float = None) -> None:
        pass
    ...
    def update_last_location(self, tenant_id: str, account_id: str, lat: float, lon: float, timestamp: float = None) -> None:
        pass
    ...
    def update_typing_baseline(self, tenant_id: str, account_id: str, cadence_ms: float) -> None:
        pass
